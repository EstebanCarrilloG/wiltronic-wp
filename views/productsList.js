...
function renderizarProductos(category, text) {
    ...
    baseDeDatos = baseDeDatos.filter(function (cc) {
      ...
      if (pedo.indexOf(text) !== -1) {
        document.getElementById("category-name").innerHTML =
          "Resultados para termino de busqueda " + `"${text}"`;
        return pedo;
      } else {
        document.getElementById("category-name").innerHTML =
          "Resultados para termino de busqueda " + `"${text}"`;
...
function nbd(baseDeDatos, productosPorPagina) {
  ...
  document.getElementById(
    "category-number"
  ).innerHTML = `Productos encontrados: (${baseDeDatos.length})`;
  ...
  function mostrarNumDePaginas(pageNumber) {
    document.querySelector(
      "#numero-pagina"
    ).innerHTML = `Pagina ${pageNumber} de ${pageCont} `;
  }
}
